...
waitUntil
{
    ...
    waitUntil { !(player getVariable ["ACE_isUnconscious", false] && (alive player)) or (time - _startedTime > 16)};

    if ((player getVariable ["ACE_isUnconscious", false]) || (player getvariable ["ca_playerisdeaddead",false]) || (!alive player)) then
    {
